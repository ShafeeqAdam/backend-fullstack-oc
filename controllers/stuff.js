...
/*voici le fichier controller, qui contient les logiques de routes, 
on créer dabord le thing avec la constante pour post, bien noter les infos dans le tableau pcq on crée,
idem quand on va updater, sauf qu'on ajoute "new".

bien sur noter exports pour y avoir accès dans les autres fichiers.

on modifie creatething car
Pour ajouter un fichier à la requête, le front-end doit envoyer les données de la requête 
sous la forme form-data et non sous forme de JSON. Le corps de la requête contient une chaîne thing, 
qui est simplement un objetThing converti en chaîne. 
Nous devons donc l'analyser à l'aide de JSON.parse() pour obtenir un objet utilisable.
Nous supprimons le champ_userId de la requête envoyée par le client 
car nous ne devons pas lui faire confiance (rien ne l’empêcherait de nous passer le userId d’une autre personne). 
...
même chose pour modyfing thing 
...
Nous avons auparavant, comme pour la route POST, supprimé le champ _userId envoyé par le client 
afin d’éviter de changer son propriétaire et nous avons vérifié 
que le requérant est bien le propriétaire de l’objet.

et pour finir deletething
...
Nous utilisons le fait de savoir que notre URL d'image contient un segment 
...
que ce soit lorsqu'un utilisateur crée un Thing, met à jour un Thing existant ou supprime un Thing !
